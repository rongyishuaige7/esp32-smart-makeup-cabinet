...
| 项目 | 当前事实 |
| :-- | :-- |
| 源码来源 | 桌面原始工程为只读来源；公开候选不反向修改原目录。来源范围见 [SOURCE_PROVENANCE](docs/SOURCE_PROVENANCE.md)。 |
...
| 固件构建 | 当前公开门禁、固件和 Flutter 构建结果只在 [VERIFICATION](docs/VERIFICATION.md) 中按实际运行记录；CI 成功仅是构建证据。 |
...
- 原始桌面工程保持只读，公开候选将凭据拆分到被忽略的本地配置，并排除缓存、IDE 状态、构建物与发行 APK。
...
- 将状态升级为“当前真机已复测”前，必须按 [VERIFICATION](docs/VERIFICATION.md) 记录日期、完整 Git commit、准确板型、模块、供电、电平、接线、每项通过/失败/未测结果和已知偏差。

详见 [PROJECT_STATUS](docs/PROJECT_STATUS.md)、[SOURCE_PROVENANCE](docs/SOURCE_PROVENANCE.md) 与 [HARDWARE_LAB_CARD](docs/HARDWARE_LAB_CARD.md)。

## 开源许可、第三方组件与问题报告

Rongyi 自有的候选源码、文档、BOM 和接线边界图以 [MIT License](LICENSE) 发布。构建所需的 ESP32 Arduino 框架、PlatformIO、Flutter SDK 和上游库由使用者自行取得；当前已核验和待复核项见 [THIRD_PARTY_NOTICES](THIRD_PARTY_NOTICES.md)。
