...
[BOM](hardware/BOM.csv)、[接线图](hardware/wiring-diagram.svg)和[硬件说明](HARDWARE.md)列出了元件与接口。接线前必须断电，确认额定电压、电流、电平、保护、公共地和负载能力；ESP32 GPIO/ADC 不得超过 3.3 V，且不能直接驱动风扇、加湿器、舵机、灯带或任何高电流/市电负载。
...
| 方法 | 路径 | 用途 |
| :-- | :-- | :-- |
| `GET` | `/api/sensors` | 读取单次传感器报告。 |
| `GET` | `/api/status` | 读取软件命令变量与本地控制开关状态。 |
| `POST` | `/api/led`、`/api/fan`、`/api/humidifier`、`/api/latch` | 在两个实验开关均设为 `1` 时提交控制命令。 |
| `GET/POST` | `/api/settings` | 读取或写入本地阈值变量。 |
| `GET` | `/api/rfid/list` | 读取本地物品记录（不含 UID）。 |
| `POST` | `/api/rfid/register` | 维护实验性 RFID 记录。 |
| 任意 | 未知路径 | 返回 `404` JSON。 |

完整字段与响应见 [PROTOCOL](docs/PROTOCOL.md)。

## 项目资料说明

本仓不包含本地 Wi-Fi 配置、私网地址、客户资料或真实传感器记录。
